...
class TurnCounter(Node):
    ...
    def on_odom_received(self, msg):
        ...
        self.get_logger().debug(
            "------received odom with heading: {:.4f}°".format(degrees(heading)))
        if self.heading_zero is not None:
            delta = to_angle(heading - self.current_heading)
            turn_inc = delta / (2 * pi)
            self.total_turn_count += turn_inc
            self.get_logger().debug(
                "New increment is: {:.4f}° ({:.4f} turns)".format(
                    degrees(delta), turn_inc))
            self.get_logger().debug(
                "Total turn count: {:.4f} ({:.4f})".format(
                    self.total_turn_count, degrees(
                        self.total_turn_count * 2 * pi)))
        else:
            self.get_logger().debug(
                "initializing Init_heading to: {:.4f}°".format(
                    degrees(heading)))
